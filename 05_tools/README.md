...
## Rounds scale with dependency, not workload

| pattern | rounds |
|---------|--------|
| parallelizable (8 `get_order` calls at once) | **1** |
| sequential (need an ID before you can fetch it) | 1 per link |
| failure + persistence prompt | until your cap |
